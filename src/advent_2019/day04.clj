...
(defn is-same-triple [[d1 d2 d3]]
  (and (is-same [d1 d2]) (is-not-same [d2 d3])))
(defn is-same-quad [[d1 d2 d3 d4]]
...
(defn is-also-valid-part2 [password]
  (let [quads (partition 4 1 password)]
    (or (is-same-triple (take 3 password))
        (is-same-triple (take 3 (reverse password)))
        (some? (some is-same-quad quads)))))
