(ns advent-2019.day19
  (:require [clojure.math.numeric-tower :refer [floor ceil]])
...
(def tractor [(assoc inputmap :rb 0) 0])
(def part1 (->> (for [x (range 50) y (range 50)] (first (second (run tractor [x y]))))
                (flatten)
                (reduce +)))

(defn beam-value
  [x y]
  (first (second (run tractor [x y]))))

(defn xrange-to-check
  [y]
  (range (int (floor (* y (/ 40 50)))) (int (ceil (* y (/ 45 50))))))

(def dxdy 99)

(defn block-within-beam
  [x y]
  (and 
   (= 1 (beam-value x y))
   (= 1 (beam-value (+ x dxdy) y))
   (= 1 (beam-value x (+ y dxdy)))))

(def part2 (for [y (range)
                 x (xrange-to-check y)
                 :when (block-within-beam x y)]
  (+ (* 10000 x ) y)))

(defn -main [& _] (println part1 (first part2)))